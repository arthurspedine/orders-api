...
func (h *Order) GetById(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, order.ErrNotExist) {
		...
		fmt.Println("failed to fid by id:", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
...
func (h *Order) UpdateById(w http.ResponseWriter, r *http.Request) {
	...
	case shippedStatus:
		if theOrder.ShippedAt != nil {
			w.WriteHeader(http.StatusBadRequest)
			errorBody.Message = "the order was already shipped"
	...
	case completedStatus:
		if theOrder.CompletedAt != nil || theOrder.ShippedAt == nil {
			w.WriteHeader(http.StatusBadRequest)
			if theOrder.ShippedAt == nil {
				errorBody.Message = "the order can't be completed because wasn't shipped"
			} else {
				errorBody.Message = "the order was already completed"
			}
			if err := json.NewEncoder(w).Encode(errorBody); err != nil {
				fmt.Println("failed to marshal", err)
				w.WriteHeader(http.StatusInternalServerError)
	...
	default:
		w.WriteHeader(http.StatusBadRequest)
		errorBody.Message = "the order status needs to be 'shipped' or 'completed'"
		if err := json.NewEncoder(w).Encode(errorBody); err != nil {
			fmt.Println("failed to marshal", err)
			w.WriteHeader(http.StatusInternalServerError)
			return
